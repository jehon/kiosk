...
export class KioskClockMainElement extends ClientElement {
  ...
  ready() {
    ...
    for (let i = 1; i <= 12; i++) {
      numbersEl.insertAdjacentHTML(
        "beforeend",
        `<text text-anchor="middle" alignment-baseline="central" fill="orange" font-size="10"
      ...
      if (!this.hasAttribute("no-date")) {
        numbersEl.insertAdjacentHTML(
          "beforeend",
          `<text text-anchor="middle" alignment-baseline="central" fill="orange" font-size="5"
    ...
    if (!this.hasAttribute("no-date")) {
      for (let i = 0; i < 60; i++) {
        numbersEl.insertAdjacentHTML(
          "beforeend",
          `<text text-anchor="middle" alignment-baseline="central" fill="red" font-size="3"
    ...
    this.arcEl = {
      total: /** @type {HTMLElement} */ (
        this.shadowRoot.querySelector("#arcTotal")
      ),
      remain: /** @type {HTMLElement} */ (
        this.shadowRoot.querySelector("#arcRemain")
      )
    };
    this.dateEl = /** @type {HTMLElement} */ (
      this.shadowRoot.querySelector("#date")
    );
    this.dowEl = /** @type {HTMLElement} */ (
      this.shadowRoot.querySelector("#dow")
    );
  }
  ...
  stateChanged(status) {
    ...
    this.hands.h.setAttribute(
      "x2",
      "" + polar2cartesianX(handLengths.h, hourAngle - Math.PI / 2)
    );
    this.hands.h.setAttribute(
      "y2",
      "" + polar2cartesianY(handLengths.h, hourAngle - Math.PI / 2)
    );

    // hand: minute
    let minuteAngle = (minute * Math.PI) / 30 + (second * Math.PI) / (30 * 60);
    this.hands.m.setAttribute(
      "x2",
      "" + polar2cartesianX(handLengths.m, minuteAngle - Math.PI / 2)
    );
    this.hands.m.setAttribute(
      "y2",
      "" + polar2cartesianY(handLengths.m, minuteAngle - Math.PI / 2)
    );

    // hand: second
    let secondAngle = (second * Math.PI) / 30;
    this.hands.s.setAttribute(
      "x2",
      "" + polar2cartesianX(handLengths.s, secondAngle - Math.PI / 2)
    );
    this.hands.s.setAttribute(
      "y2",
      "" + polar2cartesianY(handLengths.s, secondAngle - Math.PI / 2)
    );
    ...
    if (this.hasAttribute("no-date")) {
      ...
      this.dateEl.innerHTML = `${status.now.getDate()}-${
        status.now.getMonth() + 1
      }-${status.now.getFullYear()}`;
    ...
    if (status.currentTicker?.stats.end > new Date()) {
      ...
      this.arcEl.total.setAttribute(
        "d",
        describeArc(circleRadius, angleMSFromTime(start), angleMSFromTime(end))
      );
      this.arcEl.remain.setAttribute(
        "d",
        describeArc(
          circleRadius,
          angleMSFromTime(status.now),
          angleMSFromTime(end)
        )
      );
    } else {
